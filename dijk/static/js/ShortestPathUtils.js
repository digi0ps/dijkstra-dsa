...
var ShortestPathUtils = {
    ...
    makeTable: function(nodes) {
        ...
        if (nodes > ShortestPathUtils.maxNodesforDemo) {
            alert(
                "Maximum nodes for this demo is " +
                    ShortestPathUtils.maxNodesforDemo
            );
            return;
        }
            ...
            for (var td_i = 0; td_i < nodes + 1; td_i++) {
                if (tr_i === 0) {
                    if (td_i === 0) {
                        ...
                        td.appendChild(
                            document.createTextNode(
                                ShortestPathUtils.nodeNames[td_i - 1]
                            )
                        );
                    }
                } else {
                    if (td_i === 0) {
                        ...
                        td.appendChild(
                            document.createTextNode(
                                ShortestPathUtils.nodeNames[tr_i - 1]
                            )
                        );
                    } else {
                        ...
                        if (td_i > greyCol)
                            td.appendChild(
                                ShortestPathUtils.makeTextInput(
                                    td_i - 1,
                                    tr_i - 1
                                )
                            );
        ...
        target = ShortestPathUtils.clearDiv(
            ShortestPathUtils.targetDivForTable
        );

        target.appendChild(table);

        var p0 = document.createElement("p");
        p0.setAttribute("class", "find-route-p");
    ...
    makeTextInput: function(from, to) {
        ...
        input = ShortestPathUtils.addElementProperty(input, "class", "spdist input is-small cell-input");
    ...
    makeSubmitInput: function(value) {
        ...
        input = ShortestPathUtils.addElementProperty(
            input,
            "class",
            "button is-success find-route"
        );
        input = ShortestPathUtils.addElementProperty(input, "value", value);
        input.textContent = "Find Route";
        input = ShortestPathUtils.addElementProperty(
            input,
            "onclick",
            function() {
    ...
    addElementProperty: function(element, newProperty, newValue) {
        if (document.all) element[newProperty] = newValue;
        else if (document.getElementById)
            element.setAttribute(newProperty, newValue);

        if (newProperty.toLowerCase() == "class") element.className = newValue;
        if (newProperty.toLowerCase() == "onblur") element.onblur = newValue;
        if (newProperty.toLowerCase() == "onchange") element.onblur = newValue;
        if (newProperty.toLowerCase() == "onclick") element.onclick = newValue;
    ...
    callDrawGraph: function() {
        ...
        else var distances = document.querySelectorAll(".spdist");

        var source = document.getElementById("source").options[
            document.getElementById("source").selectedIndex
        ].value;
        var target = document.getElementById("target").options[
            document.getElementById("target").selectedIndex
        ].value;

        for (var i = 0; i < distances.length; i++) {
            if (
                distances[i].value != "" &&
                parseInt(distances[i].value) &&
                distances[i].value != 0
            ) {
        ...
        document.getElementById(
            ShortestPathUtils.targetDivForResults
        ).innerHTML = result;
    ...
    sortUniqueNodesFromDistances: function(distances) {
        ...
        var nodes = [
            {
                index: index,
                value: ShortestPathUtils.nodeNames[index].toString()
            }
        ];
